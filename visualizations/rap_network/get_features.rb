...
def get_collabs(root_artist, already_seen, explored, queue, all_collaborations)
    ...
    if (page_number < 10) then return end
    # Important artist!
    ...
    all_songs.each do |song|
        ...
        if (pyongs && pyongs > 5 && artists.length > 1 and (not song.title.include? "Interview") and (not song.title.include? "Tour Dates"))
            ...
            if ((not artist_names.include? "Producer Genius") and (not artist_names.include? "Rap Genius"))
                ...
                artists.each do |artist|
                    ...
                    if (not already_seen.include? name)
                        ...
                        if (name != root_name)
                            ...
                            if (collabs[name])
                                ...
                                if (not names_in_queue.include? name)
                                    queue.push(artist)
                                end
                            end
                        end
...
already_seen = ["Rap Genius", "Rock Genius", "Pop Genius", "Fashion Genius", "Producer Genius", "Billboard", "XXL Magazine", "Rolling Stone"] # Keep track of every artist we've seen so far
...
while (queue.length > 0 and explored.length < 500) do
    ...
    open('network_high_threshold.json', 'w') {|f|
